...
#define DEFINE_DUNE_OPMON_SERVICE(klass)                                                                               \
  ...
  std::unique_ptr<dunedaq::opmonlib::OpmonService> make(std::string service)                                           \
  {                                                                                                                    \
    return std::unique_ptr<dunedaq::opmonlib::OpmonService>(new klass(service));                                       \
  }                                                                                                                    \
  }
...
public:
  ...
  OpmonService& operator=(OpmonService&&) = delete;      ///< OpmonService is not move-assignable
